...
var APP=APP||{},normal=function(){for(var n="ÃÀÁÄÂÈÉËÊÌÍÏÎÒÓÖÔÙÚÜÛãàáäâèéëêìíïîòóöôùúüûÑñÇç",e="AAAAAEEEEIIIIOOOOUUUUaaaaaeeeeiiiioooouuuunncc",t={},i=0,a=n.length;i<a;i++)t[n.charAt(i)]=e.charAt(i);return function(n){for(var e=[],i=0,a=n.length;i<a;i++){var r=n.charAt(i);t.hasOwnProperty(n.charAt(i))?e.push(t[r]):e.push(r)}return e.join("")}}();APP.BuyScreen=function(){this.productConfig=JSON.parse(product.config),APP.BaseScreen.call(this,"buy"),this.totalShipping,this.currentIgv,this.totalPrice},APP.BuyScreen.constructor=APP.BuyScreen,APP.BuyScreen.prototype=Object.create(APP.BaseScreen.prototype),APP.BuyScreen.prototype.setupUI=function(){this.payBtn=$(".btn-pay"),this.form=$("form[name=buy-form]")},APP.BuyScreen.prototype.listeners=function(){APP.BaseScreen.prototype.listeners.call(this),$(".address-input").on("input change paste",function(n){$("input[name=buyerEmail]").val($("input[name=email]").val()),$("input[name=buyerFullName]").val($("input[name=firstname]").val()+" "+$("input[name=lastname]").val()),$("input[name=telephone]").val($("input[name=phone]").val()),$("input[name=shippingAddress], input[name=billingAddress]").val($("input[name=direction]").val()+" Int. "+$("input[name=departament]").val()),$("input[name=shippingCity], input[name=billingCity]").val($("input[name=province]").val()),$("input[name=zipCode]").val($("input[name=zip]").val())}),$("select[name=city]").change(this.shippingHandler),$("select[name=city]").change(),this.form.submit(this.submitHandler.bind(this)),$("input[name=direction]").geocomplete()},APP.BuyScreen.prototype.submitHandler=function(n){var e=$("input[name=buyerFullName]").val(),t=$("input[name=shippingAddress]").val(),i=$("select[name=shippingCity]").val(),a=$("input[name=shippingCity]").val(),r=$("input[name=zipCode]").val(),p=$("input[name=amount]").val(),u=$("select[name=shippingCountry]").val(),l=$("input[name=telephone]").val(),o=$("input[name=buyerEmail]").val(),s=$("input[name=signature]").val(),c=[];if(Validations.notBlank($("input[name=firstname]").val())&&c.push("Ingrese un nombre"),Validations.notBlank($("input[name=lastname]").val())&&c.push("Ingrese tus apellidos"),Validations.notBlank($("input[name=direction]").val())&&c.push("Ingrese una dirección"),Validations.notBlank($("input[name=province]").val())&&c.push("Ingrese tú provincia"),Validations.notBlank($("input[name=zip]").val())&&c.push("Ingrese el código postal"),Validations.notBlank($("input[name=phone]").val())&&c.push("Ingrese tú telefono"),Validations.notBlank($("input[name=email]").val())&&c.push("Ingrese tú correo"),c.length>0)return this.showFlash("error",c),!1;var m="/user/"+product.User.username+"/product/"+product.nameSlugify,h={name:e,direction:t,city:i,price:p,province:a,zipcode:r,country:u,phone:l,email:o},v={userId:user.id,url:m,signature:s};$("input[name=extra1]").val(JSON.stringify(h)),$("input[name=extra2]").val(JSON.stringify(v))},APP.BuyScreen.prototype.shippingHandler=function(){this.totalShipping=5,this.totalPrice=Number(product.finalPrice)+this.totalShipping,$(".subtotal").text(product.finalPrice),$(".baseprice").text(this.totalShipping),$(".totallity").text(this.totalPrice)};

...
var APP=APP||{};APP.TableSellerApplying=function(e,l){APP.BaseElement.call(this,e,"table-seller-applying",l)},APP.TableSellerApplying.prototype=Object.create(APP.BaseElement.prototype),APP.TableSellerApplying.constructor=APP.TableSellerApplying,APP.TableSellerApplying.prototype.listeners=function(){};
...
var APP=APP||{};APP.AccountApplyScreen=function(){APP.BaseScreen.call(this,"accountApply")},APP.AccountApplyScreen.constructor=APP.AccountApplyScreen,APP.AccountApplyScreen.prototype=Object.create(APP.BaseScreen.prototype),APP.AccountApplyScreen.prototype.setupUI=function(){$("input[name=direction]").geocomplete()},APP.AccountApplyScreen.prototype.listeners=function(){APP.BaseScreen.prototype.listeners.call(this)};
...
var APP=APP||{};APP.AdminSellersApplyingScreen=function(){APP.BaseScreen.call(this,"adminSellersApplying")},APP.AdminSellersApplyingScreen.constructor=APP.AdminSellersApplyingScreen,APP.AdminSellersApplyingScreen.prototype=Object.create(APP.BaseScreen.prototype),APP.AdminSellersApplyingScreen.prototype.setupUI=function(){if(this.searchBtn=$(".search-btn"),this.start=$(".start"),this.end=$(".end"),this.startValue=null,this.endValue=null,Utils.getUrlParameter("start")){var e=Utils.getUrlParameter("start");this.startValue=new Date(e),this.start.find("input").val(e.replace(/-/gi,"/"))}if(Utils.getUrlParameter("end")){var t=Utils.getUrlParameter("end");this.endValue=new Date(t),this.end.find("input").val(t.replace(/-/gi,"/"))}this.viewer=new APP.Viewer("tableSellerApplying",$(".container-items"),"infinite",data,{getTotal:function(e){$(".total").text("("+e+")")}}),this.searchBox=$(".search-box")},APP.AdminSellersApplyingScreen.prototype.listeners=function(){APP.BaseScreen.prototype.listeners.call(this),this.start.datepicker().on("changeDate",this.startDateChange.bind(this)),this.end.datepicker().on("changeDate",this.endDateChange.bind(this)),this.searchBtn.click(this.searchHander.bind(this))},APP.AdminSellersApplyingScreen.prototype.startDateChange=function(e){this.startValue=e.date},APP.AdminSellersApplyingScreen.prototype.endDateChange=function(e){this.endValue=e.date},APP.AdminSellersApplyingScreen.prototype.searchHander=function(){if(this.startValue){if(!this.endValue)return this.showFlash("error",["Ingresa una fecha de fin"]);var e=this.startValue.getMonth()+1+"-"+this.startValue.getDate()+"-"+this.startValue.getFullYear();DataApp.currentUrl=Utils.updateQueryStringParameter(DataApp.currentUrl,"start",e)}else DataApp.currentUrl=Utils.removeURLParameter(DataApp.currentUrl,"start");if(this.endValue){if(!this.startValue)return this.showFlash("error",["Ingresa una fecha de inicio"]);var t=this.endValue.getMonth()+1+"-"+this.endValue.getDate()+"-"+this.endValue.getFullYear();DataApp.currentUrl=Utils.updateQueryStringParameter(DataApp.currentUrl,"end",t)}else DataApp.currentUrl=Utils.removeURLParameter(DataApp.currentUrl,"end");DataApp.currentUrl!==window.location.href&&(Utils.changeUrl(this.id,DataApp.currentUrl),this.viewer.reset())};
...
var APP=APP||{};APP.Viewer=function(i,t,a,e,n){this.container=t,this.options=n,"carrouselItem"===i||"action"===i||"notificationItem"===i||"tableUser"===i||"tableWork"===i||"post"===i||"postMinimal"===i||"tableSellerApplying"===i||this.setupMasonry(),this.id=i,this.navigation=a,this.initialize=!0,this.fromExternal=!1,this.navigation?(this.navigationManager=new APP.NavigationManager(a),e?(this.navigationManager.navigator.currentPage=e.pagination.page,this.navigationManager.navigator.totalPages=e.pagination.pages):(this.navigationManager.navigator.currentPage=1,this.navigationManager.navigator.totalPages=1),this.navigationManager.navigator.start(),this.listeners(),e?this.addItems(e.items):(this.fromExternal=!0,this.navigationManager.navigator.gotoPage(1,!0))):e?this.addItems(e):(this.fromExternal=!0,this.navigationManager.navigator.gotoPage(1,!0))},APP.Viewer.constructor=APP.Viewer,APP.Viewer.prototype.setupMasonry=function(){var i=this;i.container.masonry({itemSelector:".grid-item",columnWidth:".grid-sizer",percentPosition:!0}),$(".discover-content").on("resetLayout",function(){console.log("expand"),i.container.masonry()})},APP.Viewer.prototype.listeners=function(){this.pageStart=this.pageLoadStartHandler.bind(this),this.navigationManager.navigator.addEventListener(Events.LOAD_START,this.pageStart),this.pageEnd=this.pageLoadEndHandler.bind(this),this.navigationManager.navigator.addEventListener(Events.LOAD_END,this.pageEnd)},APP.Viewer.prototype.pageLoadStartHandler=function(){this.container.parent().find(".bare-message").hide(),this.container.parent().find(".loading").show()},APP.Viewer.prototype.pageLoadEndHandler=function(i){this.fromExternal?this.initialize=!0:this.initialize=!1,this.addItems(i.data.items),this.options&&this.options.getTotal&&this.options.getTotal(i.data.pagination.total)},APP.Viewer.prototype.addItems=function(i){if(this.container.parent().find(".loading").show(),this.navigation!==APP.NavigationManager.PAGINATION||this.initialize||this.clean(),i.length<1){this.container.parent().find(".bare-message").hasClass("hide")&&this.container.parent().find(".bare-message").removeClass("hide");var t=this.container.parent().find(".bare-message").show().css("display");this.container.parent().find(".bare-message").show().css("display",t),this.container.parent().height("auto"),this.container.parent().find(".loading").hide()}var a,e=this,n=0,o=0,s=[];for(n;n<i.length;n++)if("action"===this.id?(("create-work"===i[n].verb||"like-work"===i[n].verb)&&(a=new APP.FeedWorkCreated(i[n],this.options)),"follow-user"===i[n].verb&&(a=new APP.FeedUserFollow(i[n],this.options))):"notificationItem"===this.id?("review-work"===i[n].verb&&(a=new APP.NotificationReview(i[n],this.options)),"follow-user"===i[n].verb&&(a=new APP.NotificationFollow(i[n],this.options)),"like-work"===i[n].verb&&(a=new APP.NotificationLike(i[n],this.options)),"request-work"===i[n].verb&&(a=new APP.NotificationRequest(i[n],this.options))):a=new(APP[Utils.capitalize(this.id)])(i[n],this.options),a.view.children().hasClass("work-card")){var r=a.view;r.attr("id",a.data.id),r.hide(),this.container.append(r),s.push(a.data.id);var h=this.container;r.imagesLoaded().progress(function(t,a){if(++o,$(a.img).parents(".grid-item"),o===i.length)var r=setTimeout(function(){for(clearTimeout(r),n=0;n<s.length;n++){var i=$("#"+s[n]);i.show(),h.masonry("appended",i),h.masonry(),e.navigationManager&&e.navigationManager.navigator.restart(),h.parent().find(".loading").hide()}},1e3)})}else"carrouselItem"===this.id||"action"===this.id||"notificationItem"===this.id||"post"===this.id||"postMinimal"===this.id||"tableUser"===this.id||"tableWork"===this.id||"tableSellerApplying"===this.id?this.container.append(a.view):(this.container.append(a.view).masonry("appended",a.view),this.container.masonry()),e.navigationManager&&e.navigationManager.navigator.restart(),this.container.parent().find(".loading").hide()},APP.Viewer.prototype.reset=function(){this.clean(),this.navigationManager.navigator.reset()},APP.Viewer.prototype.clean=function(){"carrouselItem"===this.id||"action"===this.id||"notificationItem"===this.id||"post"===this.id||"postMinimal"===this.id||"tableUser"===this.id||"tableWork"===this.id||"tableSellerApplying"===this.id?this.container.empty():this.container.masonry("remove",this.container.find(".grid-item")).masonry(),this.initialize=!1},APP.Viewer.prototype.suspend=function(){this.navigationManager.navigator.suspend(),this.navigationManager.navigator.removeEventListener(Events.LOAD_START,this.pageStart),this.navigationManager.navigator.removeEventListener(Events.LOAD_END,this.pageEnd)},APP.Viewer.prototype.restart=function(){this.navigationManager.navigator.addEventListener(Events.LOAD_START,this.pageStart),this.navigationManager.navigator.addEventListener(Events.LOAD_END,this.pageEnd),this.navigationManager.navigator.restart(),this.container.masonry()};

...
var APP=APP||{},normal=function(){for(var e="ÃÀÁÄÂÈÉËÊÌÍÏÎÒÓÖÔÙÚÜÛãàáäâèéëêìíïîòóöôùúüûÑñÇç",n="AAAAAEEEEIIIIOOOOUUUUaaaaaeeeeiiiioooouuuunncc",t={},i=0,a=e.length;i<a;i++)t[e.charAt(i)]=n.charAt(i);return function(e){for(var n=[],i=0,a=e.length;i<a;i++){var r=e.charAt(i);t.hasOwnProperty(e.charAt(i))?n.push(t[r]):n.push(r)}return n.join("")}}();APP.BuyScreen=function(){this.productConfig=JSON.parse(product.config),APP.BaseScreen.call(this,"buy"),this.totalShipping,this.currentIgv,this.totalPrice},APP.BuyScreen.constructor=APP.BuyScreen,APP.BuyScreen.prototype=Object.create(APP.BaseScreen.prototype),APP.BuyScreen.prototype.setupUI=function(){this.payBtn=$(".btn-pay"),this.form=$("form[name=buy-form]"),this.removeHandler()},APP.BuyScreen.prototype.listeners=function(){APP.BaseScreen.prototype.listeners.call(this),$(".address-input").on("input change paste",function(e){$("input[name=buyerEmail]").val($("input[name=email]").val()),$("input[name=buyerFullName]").val($("input[name=firstname]").val()+" "+$("input[name=lastname]").val()),$("input[name=telephone]").val($("input[name=phone]").val()),$("input[name=shippingAddress], input[name=billingAddress]").val($("input[name=direction]").val()+" Int. "+$("input[name=departament]").val()),$("input[name=shippingCity], input[name=billingCity]").val($("input[name=province]").val()),$("input[name=zipCode]").val($("input[name=zip]").val())}),$("select[name=city]").change(this.shippingHandler),$("select[name=city]").change(),this.form.submit(this.submitHandler.bind(this)),$("input[name=direction]").geocomplete()},APP.BuyScreen.prototype.submitHandler=function(e){var n=$("input[name=buyerFullName]").val(),t=$("input[name=shippingAddress]").val(),i=$("select[name=shippingCity]").val(),a=$("input[name=shippingCity]").val(),r=$("input[name=zipCode]").val(),o=$("select[name=shippingCountry]").val(),p=$("input[name=telephone]").val(),u=$("input[name=buyerEmail]").val(),s=$("input[name=signature]").val(),l=$("input[name=referenceCode]").val(),c=[];if(Validations.notBlank($("input[name=firstname]").val())&&c.push("Ingrese un nombre"),Validations.notBlank($("input[name=lastname]").val())&&c.push("Ingrese tus apellidos"),Validations.notBlank($("input[name=direction]").val())&&c.push("Ingrese una dirección"),Validations.notBlank($("input[name=province]").val())&&c.push("Ingrese tú provincia"),Validations.notBlank($("input[name=zip]").val())&&c.push("Ingrese el código postal"),Validations.notBlank($("input[name=phone]").val())&&c.push("Ingrese tú telefono"),Validations.notBlank($("input[name=email]").val())&&c.push("Ingrese tú correo"),c.length>0)return this.showFlash("error",c),!1;var m="/user/"+product.User.username+"/product/"+product.nameSlugify+"/submit",h={name:n,direction:t,city:i,province:a,zipcode:r,country:o,phone:p,email:u,signature:s,reference:l};h.shipping=this.totalShipping,h.price=this.price,Cookies.set("referenceCode",l,{maxAge:36e5,domain:"."+document.domain}),this.requestHandler(m,{data:JSON.stringify(h)})},APP.BuyScreen.prototype.shippingHandler=function(){this.totalShipping=5,this.totalPrice=Number(product.finalPrice)+this.totalShipping,$(".subtotal").text(product.finalPrice),$(".baseprice").text(this.totalShipping),$(".totallity").text(this.totalPrice)},APP.BuyScreen.prototype.removeHandler=function(){var e="/user/"+product.User.username+"/product/"+product.nameSlugify+"/remove";void 0!==Cookies.get("referenceCode")&&this.requestHandler(e,{reference:Cookies.get("referenceCode")})};
